# AI Wizard Guided Presets — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace free-text AI prompt input with a fully guided click-through wizard (Category → Mode → Family → Option → Confirm), expanding to 64 families / 384 preset options with enhance vs create-new modes.

**Architecture:** The wizard stays at 4 top-level steps. The "prompt" step (`AIPromptBuilder`) manages its own internal sub-navigation (mode → family → option → confirm). Presets are restructured into a nested config. Prompt templates expand from 4 to 7 to support mode-aware generation.

**Tech Stack:** React, TypeScript, Framer Motion, CSS variables (theme.css), Cloudflare Pages Functions

**Design doc:** `docs/plans/2026-03-08-ai-wizard-guided-presets-design.md`

---

## Task 1: Update Types — Add AIMode and Preset Interfaces
...
Add after the existing `AICategoryFreedom` type (line 4):
...
No other changes to this file. Existing types remain untouched.
...
git commit -m "feat(ai): add AIMode, AIStyleFamily, AICategoryPresets types"
```

---

## Task 2: Rewrite Preset Config — 64 Families, 384 Options

**Files:**
- Rewrite: `src/config/aiEnhancePresets.ts`

**Step 1: Rewrite the preset config**

Replace the entire file. The new structure:

```ts
import type { AICategory, AIMode, AIStyleFamily, AICategoryPresets } from '@/types/aiEnhance';
...
git commit -m "feat(ai): rewrite preset catalog — 64 families, 384 options, enhance + create_new modes"
```

---

## Task 3: Update Prompt Templates — Mode-Aware

**Files:**
- Modify: `functions/api/ai/_shared.ts`

**Step 1: Update prompt templates and buildConstrainedPrompt**

Replace the `PROMPT_TEMPLATES` and `buildConstrainedPrompt` section (lines 62-79) with:
...
**Step 2: Update the enhance.ts endpoint to accept mode**

In `functions/api/ai/enhance.ts`:

- Add `mode` to the body type (line 26): `mode?: string;`
- Add `mode` to destructuring (line 35): `const { walletAddress, imageBase64, category, prompt, parentEnhancementId, baseLayersJson, mode } = body;`
- Update the `buildConstrainedPrompt` call (line 67):
  ```ts
  const validMode = (mode === 'enhance' || mode === 'create_new') ? mode : 'enhance';
  const constrainedPrompt = buildConstrainedPrompt(cat, trimmedPrompt, validMode as AIMode);
  ```
- Add `AIMode` to the import from `_shared` (line 1).
...
```bash
git add functions/api/ai/_shared.ts functions/api/ai/enhance.ts
git commit -m "feat(ai): mode-aware prompt templates — enhance vs create_new"
```

---

## Task 4: Update Context — Add selectedMode State

**Files:**
- Modify: `src/contexts/AIEnhanceContext.tsx`

**Step 1: Add selectedMode to the context**

Add to the `AIEnhanceContextValue` interface (after line 21):
```ts
selectedMode: 'enhance' | 'create_new' | null;
setSelectedMode: (mode: 'enhance' | 'create_new') => void;
```

Add state in the provider (after line 67):
```ts
const [selectedMode, setSelectedMode] = useState<'enhance' | 'create_new' | null>(null);
```

Reset mode when category changes — update `selectCategory` (replace lines 123-128):
```ts
...
Update `submitEnhance` to pass mode to the API (add `mode` to the fetch body on line 147):
```ts
body: JSON.stringify({
...
Add `selectedMode` and `setSelectedMode` to the value object.
...
git commit -m "feat(ai): add selectedMode state to AIEnhanceContext"
```

---

## Task 5: Rewrite AIPromptBuilder — Guided Sub-Steps

**Files:**
- Rewrite: `src/components/generator/ai/AIPromptBuilder.tsx`

**Step 1: Rewrite the component**

Replace the entire file. The new component manages 4 internal sub-steps: `mode → family → option → confirm`.

Key behaviors:
- **Mode sub-step**: Shows "Enhance my [layer]" vs "Create new" cards. Skipped if no layer selected or background category. Uses `isSelectionPathEmpty` and `selectedLayers` from `useGenerator()` to detect layers. Maps categories to layer keys: `clothes→'Clothes'`, `head→'Head'`, `facewear→['Eye','Mask']`, `background→'Background'`.
- **Family sub-step**: Grid of family cards (emoji + label) + 🎲 Surprise Me card. Uses `AI_PRESET_CATALOG[category][mode]` to get families.
- **Option sub-step**: Grid of 6 option buttons within the selected family. Shows family name as header.
- **Confirm sub-step**: Summary card showing mode icon + category + family + option label + preview image + "Enhance — 1 credit" button. Shows "Results may vary — no charge if you discard" below button. If balance is 0, shows "No credits — Buy more" instead.
- **Back navigation**: Updates `subStep` backwards. From mode → calls `setWizardStep('category')`.
- **Surprise Me**: Calls `getRandomPreset(category, mode)`, sets family and option, jumps to confirm.
- **No textarea**. No free-text input. All prompts come from preset config.

Use `motion.button` from framer-motion for all clickable cards (same pattern as current `AICategoryPicker`). Use CSS classes: `ai-category-btn` for mode/family/option cards (already styled). Add `ai-confirm-card` class for the confirm layout.
...
git commit -m "feat(ai): rewrite AIPromptBuilder — guided sub-steps, no free text"
```

---

## Task 6: Update AIEnhanceLightbox — Dynamic Titles + Info Tooltip

**Files:**
- Modify: `src/components/generator/ai/AIEnhanceLightbox.tsx`
- Modify: `src/styles/theme.css`

**Step 1: Update lightbox title logic**

The `displayTitle` variable (lines 43-53) should account for the selected mode. Read `selectedMode` from `useAIEnhance()`. Update:
...
**Step 2: Add info tooltip button**

Add an info icon button to `ai-wizard-header-right` (next to `AICreditsDisplay`):
...
**Step 3: Add tooltip CSS to theme.css**

Add after the existing `.ai-wizard-header-right` styles (~line 3590):

```css
.ai-info-tooltip-wrapper {
  position: relative;
}
...
.ai-info-tooltip-wrapper:hover .ai-info-tooltip {
  display: block;
}
```

Also add the confirm card and disclaimer styles:
...
.ai-confirm-summary .text-secondary {
  font-size: 0.8rem;
}

.ai-confirm-disclaimer {
  font-size: 0.7rem;
  color: var(--color-text-muted);
  font-style: italic;
}
...
```bash
git add src/components/generator/ai/AIEnhanceLightbox.tsx src/styles/theme.css
git commit -m "feat(ai): dynamic wizard titles, info tooltip, confirm card styles"
```

---

## Task 7: Final Build Verification + Manual Test
...
Run: `npx wrangler pages dev --port 8788 --proxy 5173` and `npx vite --port 5173`

Test checklist:
- [ ] Open AI wizard → category picker shows 4 categories
- [ ] Pick Clothes (with a clothing layer selected) → see "Enhance" vs "Create new" mode choice
- [ ] Pick "Enhance" → see family grid with 16 families + Surprise Me
- [ ] Pick a family → see 6 options
- [ ] Pick an option → see confirm card with summary + "Enhance — 1 credit" button
- [ ] Back navigation works through all sub-steps
- [ ] Pick Background → skips mode choice, goes straight to families
- [ ] Pick Head with no head layer → skips mode choice, auto-selects create_new
- [ ] 🎲 Surprise Me → jumps to confirm with random selection
- [ ] ℹ️ info tooltip shows on hover
- [ ] Confirm disclaimer text visible
- [ ] "Enhance — 1 credit" button submits and goes to loading

**Step 3: Commit any fixes**

```bash
git add -A
git commit -m "fix(ai): address issues from manual testing"
```
...
|------|-------|-------------|
| 1 | `types/aiEnhance.ts` | Add AIMode, AIStyleFamily, AICategoryPresets types |
| 2 | `config/aiEnhancePresets.ts` | Rewrite: 64 families, 384 options, enhance + create_new |
| 3 | `_shared.ts`, `enhance.ts` | Mode-aware prompt templates (7 total) |
| 4 | `AIEnhanceContext.tsx` | Add selectedMode state |
| 5 | `AIPromptBuilder.tsx` | Rewrite: guided sub-steps, no free text |
| 6 | `AIEnhanceLightbox.tsx`, `theme.css` | Dynamic titles, info tooltip, confirm card CSS |
| 7 | — | Full build verification + manual testing |
