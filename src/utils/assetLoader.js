import { WOJAK_GENERATOR_BASE_PATH, LAYER_FOLDERS } from './constants'
...
export async function loadLayerImages(layerName) {
  ...
  // Vite serves public folder at root, so /wojak-creator/... works (folder name unchanged)
  return {}
}
...
export function getImagePath(layerName, imageName) {
  ...
  let fullPath = `${WOJAK_GENERATOR_BASE_PATH}/${folder}`
...
export function getPublicImageUrl(layerName, subfolder, fileName) {
  ...
  let path = `${WOJAK_GENERATOR_BASE_PATH}/${folder}`
  if (subfolder) {
    path += `/${subfolder}`
  }
  path += `/${fileName}.png`
