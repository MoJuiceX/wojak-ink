...
export function getTypePointsForColor(hex: string): TypePoints {
  ...
  if (effectivelyAchromatic) {
    if (l > 85) {
      return { primary: 'ICE', primaryPts: 3, secondary: 'AIR', secondaryPts: 1 };
    }
    if (l >= 60) {
      return { primary: 'METAL', primaryPts: 3, secondary: 'STONE', secondaryPts: 1 };
    }
    if (l >= 25) {
      return { primary: 'STONE', primaryPts: 3, secondary: 'GHOST', secondaryPts: 1 };
    }
    // Black (L < 25%)
