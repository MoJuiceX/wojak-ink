...
// Returns: { hasAccess: boolean, farmersPlotCount: number }
// Check if the wallet holds at least 1 Farmers Plot NFT
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  try {
    ...
    const player = await db.prepare(
      'SELECT phase1_verified, phase1_nft_count FROM game_players WHERE wallet_address = ?'
    ).bind(wallet).first<{ phase1_verified: number; phase1_nft_count: number }>();
